...
namespace VTOL.StorageNetwork
{
	...
	/// These classes are made by the user and can be registered with <see cref="ConnectionController.RegisterConnectionFilter(IConnectionFilter, int)"/>.
	...
	/// Before this list is returned, <see cref="InvalidateSiblingsPatch"/> allows all filters to cycle through this list and decide if a connection should be allowed or not.
	/// </summary>
	...
	internal static class InvalidateSiblingsPatch
	{
		...
		{
			if (!ConnectionController.Current.GetConnectionFilters(out IList<PriorityConnectionFilter> connectionFilters))
			{
				return;
			}
